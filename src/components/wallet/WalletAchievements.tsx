      ...
      <div className="grid grid-cols-3 gap-3">
        {achievements.map((achievement, index) => {
          ...
          return (
            <motion.div
              key={achievement.id}
              className={`relative aspect-square rounded-2xl p-3 flex flex-col items-center justify-center ${achievement.unlocked ? 'bg-gradient-to-br from-[#131B2E] to-[#0F1520] border-2' : 'bg-[#0A0E1A]/50 border border-white/10 opacity-50'}`}
              style={{
              ...
              {/* Icon */}
              <div
                className={`w-12 h-12 rounded-xl flex items-center justify-center mb-2 ${achievement.unlocked ? '' : 'grayscale'}`}
                style={{
