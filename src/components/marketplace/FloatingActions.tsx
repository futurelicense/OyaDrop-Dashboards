...
export function FloatingActions() {
  ...
  return (
    <div className="fixed bottom-6 right-5 z-40 flex flex-col gap-3">
      {/* Filters */}
      <motion.button
        className="w-14 h-14 bg-[#1a2a2f] border border-[#00ffcc]/30 rounded-full flex items-center justify-center shadow-lg"
        whileTap={{
          scale: 0.9
        }}
        whileHover={{
      ...
      <AnimatePresence>
        {showScrollTop &&
        <motion.button
          className="w-14 h-14 bg-gradient-to-br from-[#00ffcc] to-[#00d9ff] rounded-full flex items-center justify-center shadow-lg"
          onClick={scrollToTop}
          initial={{
            scale: 0,
            opacity: 0
