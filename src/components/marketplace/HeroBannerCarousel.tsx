  ...
  return (
    <div className="relative h-48 overflow-hidden">
        ...
        <motion.div
          ...
          <div
            ...
            <div className="relative h-full flex flex-col justify-center px-6">
              ...
              <motion.button
                className="self-start bg-white text-black px-6 py-2.5 rounded-full font-bold text-sm shadow-lg hover:scale-105 transition-transform"
                initial={{
                  y: 20,
                  opacity: 0
                }}
      ...
      <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 flex justify-between px-3 pointer-events-none">
        <motion.button
          className="w-8 h-8 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center pointer-events-auto"
          onClick={handlePrev}
          whileTap={{
            scale: 0.9
          }}>
      ...
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
        ...
        <motion.button
          key={index}
          className={`h-1.5 rounded-full transition-all ${index === currentIndex ? 'w-8 bg-white' : 'w-1.5 bg-white/40'}`}
          onClick={() => {
