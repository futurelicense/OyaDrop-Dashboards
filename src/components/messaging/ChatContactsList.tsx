...
export function ChatContactsList({
  ...
  return (
    <div className="flex flex-col h-full bg-[#0A0E1A]">
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
          {filters.map((filter) => {
            ...
            return (
              <motion.button
                key={filter.id}
                className={`flex-shrink-0 px-4 py-2 rounded-xl font-semibold text-sm transition-all ${isActive ? 'bg-gradient-to-r from-cyan-500 to-teal-500 text-white shadow-lg shadow-cyan-500/30' : 'bg-[#131B2E] text-gray-400 border border-white/10'}`}
                onClick={() => setActiveFilter(filter.id as ContactType)}
                whileTap={{
                  scale: 0.95
                }}>
      ...
      {/* Add Contact FAB */}
      <motion.button
        className="fixed bottom-20 right-4 w-14 h-14 bg-gradient-to-r from-cyan-500 to-teal-500 rounded-full shadow-2xl shadow-cyan-500/50 flex items-center justify-center z-50"
        whileHover={{
          scale: 1.1
        }}
        whileTap={{
