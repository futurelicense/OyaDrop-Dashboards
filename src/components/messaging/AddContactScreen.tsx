...
export function AddContactScreen({ onBack }: AddContactScreenProps) {
  const [activeMethod, setActiveMethod] = useState<'recent' | 'search' | 'qr'>(
    'recent'
  );
  return (
    <div className="flex flex-col h-screen bg-[#0A0E1A]">
      {/* Header */}
      <motion.header
        className="flex items-center gap-3 p-4 bg-[#131B2E] border-b border-white/10"
        initial={{
          y: -20,
          opacity: 0
        }}
      ...
      <div className="p-4 border-b border-white/10">
        <div className="grid grid-cols-3 gap-3">
          <motion.button
            className={`flex flex-col items-center gap-2 p-4 rounded-2xl transition-all ${activeMethod === 'recent' ? 'bg-gradient-to-br from-cyan-500/20 to-teal-500/20 border-2 border-cyan-500' : 'bg-[#131B2E] border-2 border-white/10'}`}
            onClick={() => setActiveMethod('recent')}
            whileTap={{
              scale: 0.98
            }}>
          ...
          <motion.button
            className={`flex flex-col items-center gap-2 p-4 rounded-2xl transition-all ${activeMethod === 'search' ? 'bg-gradient-to-br from-cyan-500/20 to-teal-500/20 border-2 border-cyan-500' : 'bg-[#131B2E] border-2 border-white/10'}`}
          ...
          <motion.button
            className={`flex flex-col items-center gap-2 p-4 rounded-2xl transition-all ${activeMethod === 'qr' ? 'bg-gradient-to-br from-cyan-500/20 to-teal-500/20 border-2 border-cyan-500' : 'bg-[#131B2E] border-2 border-white/10'}`}
              ...
              return (
                <motion.div
                  ...
                  transition={{
                    ...
                    <motion.button
                    className="w-full bg-gradient-to-r from-cyan-500 to-teal-500 text-white font-bold py-3 rounded-xl shadow-lg shadow-cyan-500/30"
                    whileHover={{
                      scale: 1.02
                    }}
                    whileTap={{
          ...
          transition={{
            ...
            <div className="bg-gradient-to-br from-[#131B2E] to-[#0F1520] rounded-3xl p-8 border border-white/10 mb-4">
              <div className="aspect-square bg-[#0A0E1A] rounded-2xl flex items-center justify-center relative overflow-hidden">
                ...
                {/* Scanning Line */}
                <motion.div
                className="absolute left-8 right-8 h-1 bg-gradient-to-r from-transparent via-cyan-400 to-transparent"
                animate={{
                  y: [-100, 100]
                }}
                transition={{
            ...
            <motion.button
            className="w-full bg-[#131B2E] border border-white/10 text-white font-bold py-4 rounded-xl"
            whileHover={{
              scale: 1.02
            }}
            whileTap={{
