...
export function GroupOrderCard({
  ...
  return (
    <motion.div
      className="max-w-[85%] bg-gradient-to-br from-[#131B2E] to-[#0F1520] rounded-2xl rounded-bl-sm p-4 border border-purple-500/30"
      initial={{
        opacity: 0,
        scale: 0.9
      }}
      ...
      <div className="space-y-2 mb-3">
        ...
        <motion.div
          key={member.id}
          className="flex items-center gap-2 p-2 bg-[#0A0E1A]/50 rounded-lg"
          initial={{
            opacity: 0,
            x: -10
          }}
          ...
          transition={{
            ...
            <p className="text-xs text-gray-500 italic">Not ordered yet</p>
            }
            </div>
            {member.confirmed &&
      ...
      <div className="flex gap-2">
        <motion.button
          className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-white/5 border border-white/10 rounded-xl text-white font-semibold text-sm"
          whileTap={{
            scale: 0.98
          }}>
