      ...
      {isOpen && <>
        ...
        stiffness: 300
            ...
            <div className="flex-1 overflow-y-auto px-6 py-6 space-y-3">
              {statuses.map((status, index) => <motion.button key={status.id} className={`w-full p-4 rounded-2xl border-2 text-left transition-all ${selectedStatus === status.id ? 'border-2' : 'border-white/10'}`} style={{
            backgroundColor: selectedStatus === status.id ? `${status.color}20` : '#0A0E1A',
            borderColor: selectedStatus === status.id ? status.color : 'rgba(255,255,255,0.1)'
          }} initial={{
            opacity: 0,
            x: -20
          }} animate={{
            ...
            scale: 0.98
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {selectedStatus === status.id && <Check className="w-5 h-5" style={{
                color: status.color
              }} />}
                  </div>
                </motion.button>)}
