        ...
        stiffness: 300
            ...
            <div className="flex-1 overflow-y-auto px-6 py-6 space-y-4">
              <div>
                ...
                <input type="text" placeholder="Enter customer name" value={formData.customerName} onChange={e => setFormData({
              ...formData,
              ...
              <div>
                ...
                <input type="tel" placeholder="080XXXXXXXX" value={formData.phone} onChange={e => setFormData({
              ...formData,
              ...
              <div>
                ...
                <textarea placeholder="Enter pickup/delivery address" value={formData.address} onChange={e => setFormData({
              ...formData,
              address: e.target.value
            })} rows={2} className="w-full bg-[#0A0E1A] text-white px-4 py-3 rounded-xl border border-white/10 focus:border-cyan-500/50 focus:outline-none placeholder:text-gray-500 resize-none" />
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {['delivery', 'pickup'].map(type => <motion.button key={type} className={`p-3 rounded-xl border-2 text-sm font-semibold transition-all ${formData.deliveryType === type ? 'bg-cyan-500/20 border-cyan-500 text-cyan-400' : 'bg-[#0A0E1A] border-white/10 text-gray-400'}`} onClick={() => setFormData({
                ...formData,
                deliveryType: type
              })} whileTap={{
                scale: 0.95
              ...
              <div>
                ...
                label: 'Express',
                color: '#FF6B00'
              }].map(option => <motion.button key={option.value} className={`p-3 rounded-xl border-2 text-sm font-semibold transition-all ${formData.urgency === option.value ? `border-[${option.color}]` : 'border-white/10'}`} style={{
                backgroundColor: formData.urgency === option.value ? `${option.color}20` : '#0A0E1A',
                color: formData.urgency === option.value ? option.color : '#9CA3AF',
                borderColor: formData.urgency === option.value ? option.color : 'rgba(255,255,255,0.1)'
              }} onClick={() => setFormData({
                ...formData,
