      ...
      {isOpen && <>
        ...
        stiffness: 300
            ...
            <div className="flex-1 overflow-y-auto px-6 py-6 space-y-3">
              {mockRiders.map((rider, index) => <motion.button key={rider.id} className={`w-full bg-[#0A0E1A] rounded-2xl p-4 border text-left ${rider.available ? 'border-white/10 hover:border-cyan-500/50' : 'border-white/5 opacity-50'}`} initial={{
            opacity: 0,
            y: 20
          }} animate={{
            opacity: 1,
            ...
            scale: rider.available ? 0.98 : 1
                  ...
                  <div className="flex items-center gap-3">
                    <div className="relative">
                      <img src={rider.avatar} alt={rider.name} className="w-14 h-14 rounded-full object-cover" />
                      {rider.available && <div className="absolute -bottom-1 -right-1 w-5 h-5 bg-green-500 rounded-full border-2 border-[#0A0E1A] flex items-center justify-center">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        ...
                        {rider.activeDeliveries === 1 ? 'delivery' : 'deliveries'}
                      </p>
                    </div>

                    {rider.available ? <div className="px-3 py-1 bg-green-500/20 border border-green-500/30 rounded-lg">
                        <span className="text-xs font-bold text-green-400">
                          Available
                        </span>
                      </div> : <div className="px-3 py-1 bg-gray-600/20 border border-gray-600/30 rounded-lg">
