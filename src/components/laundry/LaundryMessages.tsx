    ...
    return (
      <div className="flex flex-col h-full bg-[#0A0E1A]">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          ...
          <motion.div
            key={msg.id}
            className={`flex ${msg.sender === 'laundry' ? 'justify-end' : 'justify-start'}`}
            initial={{
              opacity: 0,
              y: 20
            }}
            ...
            transition={{
              ...
              <div
              className={`max-w-[75%] rounded-2xl px-4 py-3 ${msg.sender === 'laundry' ? 'bg-gradient-to-r from-cyan-500 to-teal-500 text-white rounded-br-sm' : 'bg-[#131B2E] text-white rounded-bl-sm'}`}>
              
                <p className="text-sm">{msg.text}</p>
                <p
                className={`text-xs mt-1 ${msg.sender === 'laundry' ? 'text-white/70' : 'text-gray-400'}`}>
