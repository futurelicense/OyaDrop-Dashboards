      ...
      <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
        {promos.map((promo, index) => {
          ...
          return (
            <motion.div
              key={promo.id}
              className={`relative flex-shrink-0 w-72 p-5 rounded-2xl bg-gradient-to-br ${promo.gradient} overflow-hidden`}
              initial={{
                opacity: 0,
                x: -20
              }}
              ...
              <div className="flex items-center justify-between">
                ...
                <motion.button
                  className="text-xs font-bold text-white bg-black/30 backdrop-blur-sm px-4 py-1.5 rounded-full hover:bg-black/40 transition-colors"
                  whileTap={{
                    scale: 0.95
                  }}>
