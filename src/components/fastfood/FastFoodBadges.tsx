      ...
      <div className="grid grid-cols-4 gap-3">
        {badges.map((badge, index) => {
          ...
          return (
            <motion.div
              key={badge.id}
              className={`relative aspect-square rounded-2xl p-3 flex flex-col items-center justify-center ${badge.unlocked ? 'bg-gradient-to-br from-[#131B2E] to-[#0F1520] border-2' : 'bg-[#0A0E1A]/50 border border-white/10 opacity-50'}`}
              style={{
              ...
              {/* Icon */}
              <div
                className={`w-10 h-10 rounded-xl flex items-center justify-center mb-1 ${badge.unlocked ? '' : 'grayscale'}`}
                style={{
