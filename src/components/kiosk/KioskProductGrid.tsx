    ...
    <div className="px-4 py-6 pb-24">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2 bg-[#131B2E] rounded-xl p-1 border border-white/10">
          <motion.button
            className={`p-2 rounded-lg transition-colors ${viewMode === 'grid' ? 'bg-cyan-500/20 text-cyan-400' : 'text-gray-400'}`}
          ...
          <motion.button
            className={`p-2 rounded-lg transition-colors ${viewMode === 'list' ? 'bg-cyan-500/20 text-cyan-400' : 'text-gray-400'}`}
      ...
      {cartCount > 0 &&
      <motion.button
        className="fixed bottom-6 right-4 w-14 h-14 bg-gradient-to-r from-cyan-500 to-teal-500 rounded-full shadow-2xl shadow-cyan-500/50 flex items-center justify-center z-50"
        whileHover={{
          scale: 1.1
        }}
        whileTap={{
