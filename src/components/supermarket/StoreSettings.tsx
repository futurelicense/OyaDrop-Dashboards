...
import { ChevronRight, Clock, MapPin, Bell, Users, CreditCard, Store, Shield } from 'lucide-react';
...
export function StoreSettings() {
  ...
  if (view === 'store-hours') {
    return <div className="flex flex-col h-full bg-[#0A0E1A]">
        <div className="p-4 bg-[#131B2E] border-b border-white/10">
          <motion.button className="flex items-center gap-2 text-cyan-400 font-semibold" onClick={() => setView('main')} whileTap={{
          scale: 0.95
        }}>
            ← Back
          </motion.button>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'].map((day, index) => <motion.div key={day} className="bg-gradient-to-br from-[#131B2E] to-[#0F1520] rounded-2xl p-4 border border-white/10" initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
          ...
          delay: index * 0.05
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <motion.button className="text-xs font-semibold text-cyan-400" whileTap={{
              scale: 0.95
            }}>
                  Edit
                </motion.button>
  ...
  if (view === 'auto-accept') {
    return <div className="flex flex-col h-full bg-[#0A0E1A]">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          <motion.div className="bg-gradient-to-br from-[#131B2E] to-[#0F1520] rounded-2xl p-6 border border-white/10" initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
          y: 0
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <motion.button className={`w-14 h-8 rounded-full p-1 transition-colors ${autoAccept ? 'bg-green-500' : 'bg-gray-600'}`} onClick={() => setAutoAccept(!autoAccept)} whileTap={{
              scale: 0.95
            }}>
  ...
  return <div className="flex flex-col h-full bg-[#0A0E1A]">
      <div className="flex-1 overflow-y-auto p-4">
        {settingsSections.map((section, sectionIndex) => <div key={section.title} className="mb-6">
            ...
            return <motion.button key={item.id} className="w-full bg-gradient-to-br from-[#131B2E] to-[#0F1520] rounded-2xl p-4 border border-white/10 text-left" initial={{
              opacity: 0,
              x: -20
            }} animate={{
              opacity: 1,
