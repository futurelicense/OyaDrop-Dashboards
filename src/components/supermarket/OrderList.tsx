  ...
  return <div className="flex flex-col h-full bg-[#0A0E1A]">
      ...
      <div className="p-4 border-b border-white/10">
        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
          {['All Orders', 'Pending', 'Picking', 'Ready'].map((filter, index) => <motion.button key={filter} className={`flex-shrink-0 px-4 py-2 rounded-xl font-semibold text-sm ${index === 0 ? 'bg-gradient-to-r from-cyan-500 to-teal-500 text-white' : 'bg-[#131B2E] text-gray-400 border border-white/10'}`} whileTap={{
          scale: 0.95
        }}>
                {filter}
              </motion.button>)}
