...
export function PickPackMode({
  ...
  return <div className="flex flex-col h-screen bg-[#0A0E1A]">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {items.map((item, index) => <motion.div key={item.id} className={`bg-gradient-to-br from-[#131B2E] to-[#0F1520] rounded-2xl overflow-hidden border-2 ${item.status === 'found' ? 'border-green-500' : item.status === 'unavailable' ? 'border-red-500' : 'border-white/10'}`} initial={{
        opacity: 0,
        x: -20
      }} animate={{
        opacity: 1,
        ...
        left: 0,
        right: 0
      }} dragElastic={0.2} onDragEnd={(e, info: PanInfo) => {
        ...
        if (info.offset.x > 100) handleSwipe(item.id, 'right');
            ...
            <div className="flex items-center gap-3 p-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                {item.stock === 0 && <span className="inline-block mt-1 text-xs font-bold text-red-400">
                    Out of Stock
                  </span>}
              </div>
              ...
              <div className="flex gap-2">
                <motion.button className="p-3 bg-green-500/20 border border-green-500/30 rounded-xl" whileTap={{
              scale: 0.9
            }} onClick={() => updateItemStatus(item.id, 'found')}>
                  <Check className="w-5 h-5 text-green-400" />
                </motion.button>
                <motion.button className="p-3 bg-red-500/20 border border-red-500/30 rounded-xl" whileTap={{
              scale: 0.9
            }} onClick={() => updateItemStatus(item.id, 'unavailable')}>
                  <X className="w-5 h-5 text-red-400" />
                </motion.button>
                <motion.button className="p-3 bg-cyan-500/20 border border-cyan-500/30 rounded-xl" whileTap={{
              scale: 0.9
            ...
            {/* Status Indicator */}
            {item.status !== 'pending' && <div className={`px-4 py-2 text-center text-sm font-bold ${item.status === 'found' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
      ...
      <div className="p-4 bg-[#131B2E] border-t border-white/10 space-y-2">
        <motion.button className="w-full bg-cyan-500/20 border border-cyan-500/30 text-cyan-400 font-bold py-3 rounded-xl flex items-center justify-center gap-2" whileTap={{
        scale: 0.98
      }}>
          <MessageCircle className="w-5 h-5" />
          Chat with Customer
        </motion.button>
        <motion.button className="w-full bg-gradient-to-r from-cyan-500 to-teal-500 text-white font-bold py-4 rounded-xl" whileTap={{
        scale: 0.98
      }} onClick={onComplete}>
