...
import { Search, ArrowLeft, Send, Image as ImageIcon, Package } from 'lucide-react';
...
const mockMessages: Message[] = [{
  id: '1',
  text: 'Hi! Do you have fresh tomatoes in stock?',
  sender: 'customer',
  time: '10:30 AM'
}, {
  id: '2',
  text: 'Yes, we have fresh tomatoes available. How many kg do you need?',
  sender: 'store',
  time: '10:31 AM'
}, {
  id: '3',
  text: 'I need 3kg please',
  sender: 'customer',
  time: '10:32 AM'
}];
type View = 'list' | 'chat';
  ...
  if (view === 'chat' && selectedConversation) {
    return <div className="flex flex-col h-full bg-[#0A0E1A]">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {mockMessages.map((msg, index) => <motion.div key={msg.id} className={`flex ${msg.sender === 'store' ? 'justify-end' : 'justify-start'}`} initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
          ...
          delay: index * 0.1
              ...
              <div className={`max-w-[75%] rounded-2xl px-4 py-3 ${msg.sender === 'store' ? 'bg-gradient-to-r from-cyan-500 to-teal-500 text-white rounded-br-sm' : 'bg-[#131B2E] text-white rounded-bl-sm'}`}>
                <p className="text-sm">{msg.text}</p>
                <p className={`text-xs mt-1 ${msg.sender === 'store' ? 'text-white/70' : 'text-gray-400'}`}>
                  {msg.time}
                </p>
              </div>
            </motion.div>)}
        ...
        <div className="px-4 py-2 border-t border-white/10 bg-[#131B2E]">
          <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
            {['Item available', 'Out of stock', 'Ready in 10 mins', 'Order ready'].map(reply => <motion.button key={reply} className="flex-shrink-0 px-3 py-2 bg-[#0A0E1A] border border-white/10 rounded-xl text-xs font-semibold text-white" whileTap={{
            scale: 0.95
          }}>
                {reply}
              </motion.button>)}
