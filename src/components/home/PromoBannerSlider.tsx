    ...
    <div className="px-5 py-6">
      <div className="relative">
        <AnimatePresence mode="wait">
          <motion.div
            key={currentIndex}
            className={`relative bg-gradient-to-br ${banners[currentIndex].gradient} rounded-2xl p-6 overflow-hidden shadow-xl`}
            initial={{
              opacity: 0,
              x: 100
            }}
        ...
        <div className="absolute top-1/2 -translate-y-1/2 left-2 right-2 flex justify-between pointer-events-none">
          <motion.button
            className="w-8 h-8 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center pointer-events-auto"
            onClick={handlePrev}
            whileTap={{
              scale: 0.9
            }}>
      ...
      <div className="flex items-center justify-center gap-2 mt-4">
        ...
        <motion.button
          key={index}
          className={`h-2 rounded-full transition-all ${index === currentIndex ? 'w-6 bg-[#00ffcc]' : 'w-2 bg-white/30'}`}
