...
export function BeautyRequestSheet({
  ...
  return <ServiceRequestSheet isOpen={isOpen} onClose={onClose} title="Beauty Services" subtitle="Hair, nails, spa & more" color="#FF6B00" icon={<Sparkles className="w-6 h-6 text-orange-400" />} onSubmit={handleSubmit} submitLabel="Find Beauty Providers">
      <div className="space-y-6">
        ...
        <div>
          ...
          {categories.map(category => <div key={category} className="mb-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                {beautyServices.filter(s => s.category === category).map(service => <motion.button key={service.id} className={`p-3 rounded-xl border-2 transition-all ${selectedServices.includes(service.id) ? 'bg-orange-500/20 border-orange-500' : 'bg-[#0A0E1A] border-white/10'}`} onClick={() => toggleService(service.id)} whileTap={{
              scale: 0.98
            }}>
                      <div className="text-xl mb-1">{service.icon}</div>
                      <p className={`text-xs font-semibold ${selectedServices.includes(service.id) ? 'text-white' : 'text-gray-400'}`}>
                        {service.name}
                      </p>
                    </motion.button>)}
              </div>
        ...
        {selectedServices.length > 0 && <div className="bg-orange-500/10 border border-orange-500/30 rounded-xl p-4">
            ...
            return <span key={serviceId} className="text-xs bg-orange-500/20 text-orange-300 px-2 py-1 rounded-lg">
