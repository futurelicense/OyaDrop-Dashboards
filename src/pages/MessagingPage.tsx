...
export function MessagingPage({ onMenuClick }: MessagingPageProps) {
  ...
  return (
    <>
      <div className="flex flex-col h-screen bg-[#0A0E1A]">
        {/* Header */}
        <motion.header
          className="flex items-center justify-between p-4 bg-[#131B2E] border-b border-white/10"
          initial={{
            y: -20,
            opacity: 0
          }}
        ...
        <div className="flex border-b border-white/10 bg-[#131B2E]">
          ...
          map((tab) => {
            ...
            return (
              <motion.button
                key={tab.id}
                className={`flex-1 flex items-center justify-center gap-2 py-3 relative ${isActive ? 'text-cyan-400' : 'text-gray-400'}`}
        ...
        <div className="flex-1 overflow-hidden">
          {activeTab === 'chats' &&
          <ChatConversationsList onSelectChat={handleSelectChat} />
          }
          {activeTab === 'contacts' &&
          <ChatContactsList
            onAddContact={() => setCurrentView('add-contact')}
            onSelectContact={handleSelectContact} />

          }
          {activeTab === 'notifications' &&
          <UpdatesFeed
            onCreateUpdate={() => setShowCreateUpdate(true)}
